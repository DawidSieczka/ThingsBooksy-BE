...
public static class ModuleLoader
{
    public static IList<Assembly> LoadAssemblies(IConfiguration configuration, string modulePart)
    {
        ...
        var mySpotFiles = files.Where(x => Path.GetFileName(x).StartsWith("ThingsBooksy.")).ToList();
        mySpotFiles.ForEach(x => assemblies.Add(AppDomain.CurrentDomain.Load(AssemblyName.GetAssemblyName(x))));
